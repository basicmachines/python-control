...
def vehicle_update(t, x, u, params=None):
    ...
    params : dict
        Dictionary of parameter values.  If not provided, default
        value used.
...
def pi_update(t, x, u, params=None):
    if params is None:
        params = {}
    ...
    z = x[0]                    # integrated error  TODO: This is not used
...
def pi_output(t, x, u, params=None):
    if params is None:
        params = {}
    # Get the controller parameters that we need
...
def cruise_plot(sys, t, y, t_hill=5, vref=20, antiwindup=False,
    ...
    v_min = vref-1.2
    v_max = vref+0.5
    v_ind = sys.find_output('v')
    u_min = 0
    u_max = 2 if antiwindup else 1
    u_ind = sys.find_output('u')

    # Make sure the upper and lower bounds on v are OK
    while max(y[v_ind]) > v_max:
        v_max += 1
    while min(y[v_ind]) < v_min:
        v_min -= 1
...
def sf_update(t, z, u, params=None):
    y, r = u[1], u[2]
    return y - r


def sf_output(t, z, u, params=None):
    if params is None:
        params = {}
    # Get the controller parameters that we need
    K = params.get('K', 0)
